...
|---|---|---|---|
| Ethereum Sepolia (11155111) | Enabled — `deployments/ethereum-sepolia.json` | Wallet login, CAFE faucet, liquid deposit/redeem/claim, reward minting, agentic commerce (ERC-8004 / ERC-7683 / ERC-8183), marketplace payment (native ETH checkout, verified against the legacy pool wallet) | Session-key payments, legacy exit |
| BNB Smart Chain Testnet (97) | Enabled — `deployments/bsc-testnet.json` | Wallet login, CAFE faucet, liquid deposit/redeem/claim, reward minting, agentic commerce | Session-key payments, marketplace payment (blocked on native BNB pricing, see below) |
...
One capability nuance is deliberate today: for EVM chains the manifest loader grants a fixed set (wallet login, liquid staking, faucet, reward minting) and reads `agenticCommerce`, `agenticSessionPayments`, `marketplacePayment`, and `legacyExit` from the manifest's `capabilities` object — the manifest is the single source of truth. A flag only takes effect when the deployment it needs is present: `marketplacePayment` requires an escrow (or the legacy pool) and `legacyExit` requires a legacy pool, or `ChainRegistry.Validate` rejects the manifest. `legacyExit` stays off on every chain — see the capability roadmap in [`docs/multichain-liquid-staking-plan.md`](docs/multichain-liquid-staking-plan.md).

`marketplacePayment` is live on Ethereum Sepolia: `Checkout.razor` is chain-registry-driven (no longer hardcoded to a single network), pays native currency to the manifest's `legacyPool` address, and verifies the transfer server-side (`EvmMarketplacePaymentGateway`) before recording the order — proven with a real Sepolia transaction. It stays off on BSC Testnet because the order total is still priced off a static ETH/USD rate; converting that total to BNB at the same rate would charge the wrong amount, and the persisted receipt model still names/formats the value as ETH. Checkout therefore refuses non-ETH native currencies. The exact implementation and public-test gates are tracked in [`docs/bsc-testnet-marketplace-follow-up.md`](docs/bsc-testnet-marketplace-follow-up.md).
