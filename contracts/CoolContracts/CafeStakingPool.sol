...
contract CafeStakingPool is Ownable, ReentrancyGuard {
    ...
    // Alias for the front-end, which reads the pool's total via totalSupply().
    function totalSupply() external view returns (uint256) {
        return totalStaked;
    }

    // Alias for the front-end, which derives the displayed APR from aprBasisPoints().
    function aprBasisPoints() external view returns (uint256) {
        return annualRewardBps;
    }
