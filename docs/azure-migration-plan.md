...
Phase 3 is complete and running in production.

## Phase 4 — CI/CD (done, opened as a PR)

Rewrote `.github/workflows/ci.yml`'s deploy job (`build-test` left unchanged): builds and pushes both images via `az acr build` (no Docker needed on the runner, builds natively for `linux/amd64`), authenticates via OIDC (`azure/login@v2`, no client secret/password anywhere), runs EF Core migrations directly from the runner against the real Postgres (temporarily opening a firewall rule for the runner's IP and always closing it via `if: always()` — same "authorize runner IP, do the thing, revoke it" pattern already documented in this repo's `docs/github-actions-ec2-deploy.md` for the old SSH flow, just applied to Postgres instead), then `az containerapp update` for both apps, then polls `/health` on the real FQDN.

**Deliberately used a separate CI/CD identity, not the app's runtime identity.** Added `infra/modules/cicdIdentity.bicep`: a second user-assigned managed identity with a GitHub OIDC federated credential (subject `repo:AlejoReyna/ArtisanalBrew:ref:refs/heads/main`), granted narrowly-scoped roles: `AcrPush` (registry-scoped), `Contributor` (scoped to just the Postgres server resource, for firewall rules), `Key Vault Secrets User` (vault-scoped, to fetch the DB connection string), and `Container Apps Contributor` (scoped to each of the two Container Apps individually, not the resource group). The app's own runtime identity was left untouched — still only has its original minimal `AcrPull`/`Key Vault Secrets User`/`Storage Blob Data Contributor`/Service Bus roles. Deployed and verified: federated credential and all 5 role assignments confirmed via `az identity federated-credential list` and `az role assignment list`.

**GitHub repo variables set** (non-secret identifiers, not credentials — nothing sensitive was ever stored in GitHub): `AZURE_CICD_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`.

**Git incident during this phase, for the record:** a parallel Claude Code session (Fable 5, co-authoring unrelated wallet-dashboard UI work on a `feature/crypto-ui-rework` branch) was operating in this same working directory concurrently. Two effects: (1) my `git checkout -b azure-migration-cicd` silently branched from whatever was checked out at that exact moment (`feature/crypto-ui-rework`) instead of `persistent-component-state`, because branch switches were racing in the same physical repo; (2) that same branch name (`azure-migration-cicd`) got reset to `persistent-component-state`'s tip by the other session shortly after, for its own unrelated purposes — a pure naming collision, not data loss (the commit survived in the reflog). Recovered by verifying the commit's actual content via `git show <sha>:<path>` (confirmed correct), confirming zero file overlap between the two branches' commits, cherry-picking the migration commit onto a freshly-named branch (`azure-container-apps-migration`) based directly on `persistent-component-state`, and pushing immediately to origin to get the work off local shared state. **Takeaway: don't reuse generic branch names when another agent/session may be active in the same repo — verify `git branch --show-current` and `git merge-base` immediately after any `checkout -b`, not just before.**
