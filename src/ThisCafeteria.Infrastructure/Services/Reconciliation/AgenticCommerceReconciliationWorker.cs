    ...
    ILogger<AgenticCommerceReconciliationWorker> logger) : BackgroundService
{
    ...
    public async Task ReconcileOnceAsync(ChainDefinition chain, CancellationToken cancellationToken)
    {
        ...
        var projections = scope.ServiceProvider.GetRequiredService<IAgenticCommerceProjectionBatch>();
        ...
        {
            await applicator.ApplyEventAsync(projections, chain, escrowAddress, evt, cancellationToken).ConfigureAwait(false);
        }

        foreach (var evt in registryEvents.OrderBy(e => e.BlockNumber).ThenBy(e => e.LogIndex))
        {
            await applicator.ApplyRegistryEventAsync(projections, chain, registryAddress, evt, cancellationToken).ConfigureAwait(false);
        }
