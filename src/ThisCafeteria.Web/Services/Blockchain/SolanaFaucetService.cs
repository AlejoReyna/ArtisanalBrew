...
public sealed record SolanaFaucetClaimResult(bool Success, string? Signature, string? ExplorerUrl, decimal Amount, string? Error, DateTime? NextClaimAtUtc);
    ...
    ILogger<SolanaFaucetService> logger) : ISolanaFaucetService
{
    ...
    public async Task<SolanaFaucetClaimResult> ClaimAsync(string chainKey, string wallet, CancellationToken cancellationToken = default)
    {
        ...
        dbContext.SolanaFaucetClaims.Add(new SolanaFaucetClaim
        {
            ChainKey = chain.Key,
    ...
    {
        var last = await dbContext.SolanaFaucetClaims.AsNoTracking()
            .Where(claim => claim.ChainKey == chainKey && claim.WalletAddress == wallet)
