    ...
    ILogger<ChainReconciliationSupervisor> logger) : BackgroundService
{
    ...
    private static StakingLedgerEntry ToEntry(ChainDefinition chain, FilterLog log, string wallet, string action, BigInteger assets, BigInteger shares, BigInteger rewards) => new()
    {
        WalletAddress = wallet,
        ChainKey = chain.Key,
        Family = chain.FamilyName,
        ActionType = action,
        Amount = FromWei(assets != 0 ? assets : rewards),
        AssetAmount = FromWei(assets),
        ShareAmount = FromWei(shares),
        RewardAmount = FromWei(rewards),
        RawAssetAmount = assets.ToString(),
        RawShareAmount = shares.ToString(),
        RawRewardAmount = rewards.ToString(),
        TransactionHash = log.TransactionHash?.ToLowerInvariant() ?? string.Empty,
        OperationIndex = checked((int)(log.LogIndex?.Value ?? 0)),
        ChainId = chain.EvmChainId ?? 0,
        NetworkName = chain.DisplayName,
        PaymentTokenContract = chain.Deployment.Cafe,
        StakingPoolContract = chain.Deployment.LiquidVault,
        AssetIdentifier = chain.Deployment.Cafe,
        ReceiptIdentifier = chain.Deployment.StCafe,
        RewardIdentifier = chain.Deployment.Coffee,
        VaultOrProgramIdentifier = chain.Deployment.LiquidVault,
        BlockOrSlot = (long)(log.BlockNumber?.Value ?? 0),
        VerificationState = "reconciled",
        Verified = true,
        RecordedAtUtc = DateTime.UtcNow,
        OccurredAtUtc = DateTime.UtcNow,
        ExplorerUrl = string.Format(chain.ExplorerTransactionTemplate, log.TransactionHash)
    };
