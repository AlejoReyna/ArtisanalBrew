    ...
    ILogger<SolanaReconciliationSupervisor> logger) : BackgroundService
{
    ...
    internal static IEnumerable<StakingLedgerEntry> Decode(ChainDefinition chain, string signature, long slot, JsonElement transaction)
    {
        ...
        foreach (var anchorEvent in events)
        {
            ...
            yield return new StakingLedgerEntry { WalletAddress = wallet, ChainKey = chain.Key, Family = chain.FamilyName, ActionType = action, Amount = action is "claim" or "reward_funding" ? FromRaw(reward, chain.Deployment.CoffeeDecimals) : FromRaw(assets, chain.Deployment.CafeDecimals), AssetAmount = FromRaw(assets, chain.Deployment.CafeDecimals), ShareAmount = FromRaw(shares, chain.Deployment.StCafeDecimals), RewardAmount = FromRaw(reward, chain.Deployment.CoffeeDecimals), RawAssetAmount = assets.ToString(CultureInfo.InvariantCulture), RawShareAmount = shares.ToString(CultureInfo.InvariantCulture), RawRewardAmount = reward.ToString(CultureInfo.InvariantCulture), TransactionHash = signature, OperationIndex = anchorEvent.LogIndex, NetworkName = chain.DisplayName, PaymentTokenContract = chain.Deployment.Cafe, StakingPoolContract = chain.Deployment.Program, AssetIdentifier = chain.Deployment.Cafe, ReceiptIdentifier = chain.Deployment.StCafe, RewardIdentifier = chain.Deployment.Coffee, VaultOrProgramIdentifier = chain.Deployment.Program, BlockOrSlot = slot, VerificationState = "reconciled", Verified = true, RecordedAtUtc = DateTime.UtcNow, OccurredAtUtc = DateTime.UtcNow, ExplorerUrl = string.Format(chain.ExplorerTransactionTemplate, signature) };
        }
    }
